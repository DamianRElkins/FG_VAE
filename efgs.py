...
def psmi_can(s):
    try:
        ...
        s = s.replace("[Tl]+", "[Tl+]")
        s = s.replace("[Par]", "[U]").replace("[Par+]", "[U+]").replace("[Par-]", "[U-]")
        ...
        s = s.replace("[U]", "[Par]").replace("[U+]","[Par+]").replace("[U-]","[Par-]")
        return s
    except Exception as e:
        print(f"[ERROR] Failed psmi_can for input: {repr(s)}\nReason: {e}")
        return None  # or raise, or keep original input
